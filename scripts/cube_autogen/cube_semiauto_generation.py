...
INELOD = Namespace("http://lod.ine.es/voc/cubes/")
...
INELOD_VOC = Namespace("http://lod.ine.es/voc/cubes/vocabulary/")
...
def add_POM_from_csv(file_path):
    ...
    with open(file_path, mode='r', encoding='utf-8') as csvfile:
        ...
        for column in columns:
            # Dimension columns
            if column in variables_dict and column not in medidas_set and column not in measures_dict:
                sdmx_dim = variables_dict[column]
...
def add_mappings_from_csv(file_path):
    ...
    g_mappings.add((subject_map_bnode, RR.template, Literal("https://lod.ine.es/voc/cubes/" + file_name + "/o{index}")))
