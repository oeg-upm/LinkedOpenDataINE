...
def _add_series_triplesmap(table_name, dimensions, parameters, g_mappings):
    ...
    # Esta información es redundate con respecto a qb:measure y sdmx-attribute:decimals anterior (por eso el false de los ifs)
    if (parameters.get("det") == "2" or parameters.get("det") == "1") and False:
        add_pom_parenttpm(series, EX["hasScale"], INELOD[table_name + "_Escala"], "Escala.Id", "Id", g_mappings)
        add_pom_parenttpm(series, SDMX_ATTRIBUTE["unitMeasure"], INELOD[table_name + "_Unidad"], "Unidad.Id", "Id", g_mappings)
    elif False:
    ...
    if "M" in parameters.get("tip"):
        add_pom_parenttpm(series, QB["ComponentSpecification"], INELOD[table_name + "_Meta"], "MetaData.Id", "Id", g_mappings)
...
def _add_observations_triplesmap(table_name, parameters, g_mappings):
    ...
    if "A" not in parameters.get("tip"): 
        #add_pom_ref(observations, SDMX_ATTRIBUTE["confStatus"], "Data.Secreto", datatype=XSD.boolean, g_mappings=g_mappings) 
        add_pom_obj_iri(observations, SDMX_ATTRIBUTE["confStatus"], "http://purl.org/linked-data/sdmx/2009/code#confStatus-{Data.Secreto}", g_mappings)
    ...
    add_pom_ref(observations, SDMX_ATTRIBUTE.obsStatus, "Data.TipoDato.Nombre", g_mappings)
...
def _add_scale_triplesmap(table_name, parameters, g_mappings):
    ...
    if "A" not in parameters.get("tip"):
        add_pom_ref(scale, DCT["identifier"], "Id", datatype=XSD.integer, g_mappings=g_mappings)
...
def _add_tipodato_triplesmap(table_name, parameters, g_mappings):
    ...
    if "A" not in parameters.get("tip"):
        add_pom_ref(tipo_dato, DCT["identifier"], "Data.TipoDato.Id", g_mappings, datatype=XSD.integer)
#OLD
def _add_periodo_triplesmap(table_name, parameters, g_mappings):
    ...
    if "A" not in parameters.get("tip"):
        add_pom_ref(periodo, SKOS["prefLabel"], "Data.Periodo.Nombre_largo", g_mappings)
        ...
        add_pom_ref(periodo, DCT["identifier"], "Data.Periodo.Id", g_mappings, datatype=XSD.integer)
        add_pom_ref(periodo, SDMX_MEASURE["obsValue"], "Data.Periodo.Valor", g_mappings)
...
def _add_meta_triplesmap(table_name, parameters, g_mappings):
    ...
    add_pom_ref(meta, SKOS["notation"], "Codigo", g_mappings)
    add_pom_ref(meta, SKOS["note"], "Nota", g_mappings)

    if parameters.get("det") == 2:
        add_pom_parenttpm(meta, QB["DimensionProperty"], INELOD[table_name + "_Variable"], "Variable.Id", "Variable.Id", g_mappings)
    else:
        add_pom_ref(meta, QB["DimensionProperty"], "Variable.Id", g_mappings)
